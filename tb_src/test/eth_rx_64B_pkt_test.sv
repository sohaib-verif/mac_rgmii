...
import eth_rx_pkg::eth_rx_64B_pkt_seq;
class eth_rx_64B_pkt_test extends uvm_test;
  `uvm_component_utils(eth_rx_64B_pkt_test)

  top_env     eth_env     ;
  // eth_tx_sequence_lib    eth_seq_lib ;
  eth_rx_64B_pkt_seq rx_seq;
  eth_ref_model       ref_mod     ;

  
  function new(string name = "eth_rx_64B_pkt_test", uvm_component parent);
  ...
  function void build_phase(uvm_phase phase);
    ...
    `uvm_info(get_type_name(), "eth_rx_64B_pkt_test build phase started", UVM_LOW)

    // eth_seq_lib       = eth_tx_sequence_lib::type_id::create("eth_seq_lib");
    rx_seq = eth_rx_64B_pkt_seq::type_id::create("rx_seq");
