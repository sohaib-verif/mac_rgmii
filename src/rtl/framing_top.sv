...
//=============================================================================
// FRAMING TOP - Ethernet Frame Processing and MAC Filtering
//=============================================================================
// This module provides Ethernet frame processing, MAC address filtering, and
// integration between the system interface and RGMII physical layer.
//
// PURPOSE:
//   - MAC address filtering (unicast, broadcast, multicast, promiscuous)
//   - Ethernet frame processing and validation
//   - Integration of MAC, RGMII, and configuration registers
//   - Bridge between 8-bit AXI-Stream and RGMII physical layer
//
// KEY FEATURES:
//   - **MAC Address Filtering**: Filters received frames based on destination MAC
//     • Accepts: Own MAC address, Broadcast (FF:FF:FF:FF:FF:FF), Multicast (01:xx:xx:xx:xx:xx)
//     • Rejects: Other unicast addresses (unless promiscuous mode enabled)
//   - **Promiscuous Mode**: When enabled, accepts ALL frames regardless of MAC
//   - **Configuration Registers**: MAC address, promiscuous mode, statistics
//   - **RGMII Integration**: Complete RGMII PHY interface with MAC
//
// ARCHITECTURE:
//   TX: AXI-Stream (8-bit) → MAC → RGMII → PHY
//   RX: AXI-Stream (8-bit) ← MAC Filtering ← RGMII ← PHY
//
// MAC ADDRESS FILTERING (RX Path):
//   1. Frame arrives from RGMII
//   2. Extract destination MAC address (first 6 bytes)
//   3. Compare with configured MAC address (from registers)
//   4. Check for broadcast (all FF) or multicast (LSB of first byte = 1)
//   5. If match OR promiscuous mode: Output frame to AXI-Stream
//   6. If no match: Discard frame silently
//
// REGISTER MAP (via REG_BUS):
//   0x0: MAC_ADDR_LOW  [31:0] - Lower 32 bits of MAC address
//   0x4: MAC_ADDR_HIGH [15:0] - Upper 16 bits of MAC address
//        [16] - Promiscuous mode enable
//   0x8: STATUS - Link status, frame counters
//   0xC: CONTROL - Enable, reset, etc.
//
// DATA FLOW:
//   TX: tx_axis_req_i → rgmii_soc → RGMII signals → PHY
//   RX: PHY → RGMII signals → rgmii_soc → MAC Filter → rx_axis_req_o
//=============================================================================
